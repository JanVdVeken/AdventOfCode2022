...
namespace AoC2022Days.DayHelpers.Day14
{
    public class Cave
    {
        ...
        public Cave(List<CaveLine> inputs, CavePoint sandStartingPoint)
        {
            ...
            _caveAmountOfCols = pointsToWallUp.Max(p => p.X)+1;
            _lowestUsedCol = pointsToWallUp.Min(p => p.X)-1;
            _caveAmountOfRows = pointsToWallUp.Max(p => p.Y)+1;
        ...
        public int CalculateSandBeforeOverflow()
        {
            ...
            while (continueDroppingSand)
            {
                ...
                while (true)
                {
                    ...
                    if (oneLower.Y >= _caveAmountOfRows) continueDroppingSand = false;
                    break;
                }
            }
            return count -1;
