...
public abstract class Day
{
    ...
    {        
        ...
        stopwatch.Stop();
        Console.WriteLine($"Puzzle1: {(double)stopwatch.ElapsedTicks / TimeSpan.TicksPerMillisecond}ms \t=> {p1Result}");
        stopwatch.Reset();
        stopwatch.Start();
        var p2Result = Puzzle2(_input);
        stopwatch.Stop();
        Console.WriteLine($"Puzzle2: {(double)stopwatch.ElapsedTicks / TimeSpan.TicksPerMillisecond}ms \t=> {p2Result}");
    }
}